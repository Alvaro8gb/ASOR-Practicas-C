...
int main(int argc, char **argv){
    ...
    do{
        ...
        retval = select(udp_sd+1, &rfds, NULL, NULL, NULL);
            
        if (retval == -1) handle_error("Error in slect()");
           
        if(FD_ISSET(udp_sd, &rfds)){
        ...
        if(FD_ISSET(STDIN_FILENO, &rfds)){
            printf("Reading from Standar Input\n");
            bytes_read = read(STDIN_FILENO, buf, 2);

            if( bytes_read == -1) handle_error("Error redinng from input");

            printf("%ld bytes de %s\n", bytes_read, "Input");
        ...
        if(FD_ISSET(STDIN_FILENO, &rfds)){
            write(STDIN_FILENO, msg, bytes_to_client);
        }
